...
public class UserDaotest {
	public static void main(String[] args) {
		...
		List<User> cardList2 = dao.select(new User(0,"","","","","","","", 0));  // 田、県を含むものをリストで出す↓
		for (User card : cardList2) {
			...
			System.out.println("company：" + card.getUser_word_s());
			System.out.println("address：" + card.getUser_word_t());
			System.out.println("tell：" + card.getUser_type());
		}

		// insert()のテスト
		System.out.println("---------- insert()のテスト ----------");
		User insRec = new User(0,"test","test","","","","","", 0);
		...
		User upRec = new User(0,"test","test","","","","","", 0);
		if (dao.update(upRec)) {
